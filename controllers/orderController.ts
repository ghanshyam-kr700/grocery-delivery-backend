...
export const createOrder = async (req: Request, res: Response) => {
    ...
    if (!items || items.length === 0) {
        return res.status(400).json({ message: "no orders items" });
    }

    // loook up actual from the data base
    const productsId = items.map((i: any) => i.product);
    ...
    for (const item of items) {
        ...
        if (!product || (product.stock ?? 0) < item.quantity) {
            return res.status(404).json({ mssage: "product of stock " });
        }
    }
    ...
    for(const item of orderItems){
        await inngest.send({name:"inventory stock updated ",data:{productsId:item.product}})
    }
