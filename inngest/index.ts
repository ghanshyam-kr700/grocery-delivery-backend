...
const checkLowStock = inngest.createFunction(
    ...
    async ({ event, step }) => {
        const { productsId } = event.data;

        const product = await step.run('fetch-product', async () => {
            return await prisma.product.findUnique({ where: { id: productsId } })
        })
...
const sendMonthlyOffers = inngest.createFunction(
    ...
    async ({ step }) => {
        ...
        // send emails in batches of 10 avoid overwhelming mail server 
        const batchSize = 10;
...
const autoAssignRider = inngest.createFunction({
    ...
    const result = await step.run("assign-rider", async () => {
        ...
        //skip if order doesnt exist ,already assigned , or cancelled
        ...
        if(!availableRider) return {skipped:true,reason:"No riders availble"}
